...
make_libinfo(void)
{
    ...
    insert_libhdf5_settings(output);
}
...
information about the library build configuration\n";
     ...
     */
    fprintf(output, "/* Generated automatically by H5make_libsettings -- do not edit */\n\n\n");
	...
	fprintf(output, " *\t\t\t");
	if(real_name[0])
            fprintf(output, "%s <", real_name);
#ifdef H5_HAVE_GETPWUID
	if(pwd)
            HDfputs(pwd->pw_name, output);
#endif
	if(host_name[0])
            fprintf(output, "@%s", host_name);
	if(real_name[0])
            fprintf(output, ">");
	HDputchar('\n');
    } /* end if */
    ...
    fprintf(output, " *\tIt was generated by code in `H5make_libsettings.c'.\n");
